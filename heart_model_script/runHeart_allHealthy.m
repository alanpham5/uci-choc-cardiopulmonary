...
%% User parameters
numPatients = 62;
outputFilename = 'healthy_patient_heart_sim_results.csv';
...
% Healthy ranges (typical adult)
BSA         = 1.7 + 0.2*randn(numPatients,1);                % 1.5–2.0 m² approx
HR          = 60 + 10*randn(numPatients,1);                  % 50–75 bpm
SBP         = 115 + 10*randn(numPatients,1);                 % 105–125 mmHg
DBP         = 70  + 8*randn(numPatients,1);                  % 60–80 mmHg
HI          = 2.5 + 0.1*randn(numPatients,1);                % normal Haller index (~2.5)

% Normal LV/RV indexed volumes
LVEDVi = 75 + 8*randn(numPatients,1);
LVESVi = 30 + 5*randn(numPatients,1);
RVEDVi = 85 + 10*randn(numPatients,1);
RVESVi = 35 + 6*randn(numPatients,1);

% Healthy compressions: none
IVCcomp = zeros(numPatients,1);
RVcomp  = zeros(numPatients,1);

% Healthy reserve metrics
LVEF        = 60 + 3*randn(numPatients,1);
RVEF        = 60 + 4*randn(numPatients,1);
LVmassIdx   = 75 + 10*randn(numPatients,1);
Eesep       = 8  + 1*randn(numPatients,1);
O2PulsePct  = 100 + 10*randn(numPatients,1);
VO2maxPct   = 100 + 10*randn(numPatients,1);
CCI         = 1.2 + 0.3*randn(numPatients,1);
SternalTor  = 5 + 3*randn(numPatients,1);
RAsz        = 18 + 5*randn(numPatients,1);
LAsz        = 22 + 5*randn(numPatients,1);


%% List of parameters we need
%{
HR (or max HR for calculation)
LVEDVi, LVESVi, BSA
RVEDVi, BSA
SBP, DBP (for pulse pressure)
HI
IVCComp, RVComp (flag to scale resistance if true)
LVEF
EEsep, LA_size (for R7)
%}


%% --- File setup ---
%infile  = fullfile(proj.RootFolder, 'choc-data-all-temp.csv');
healthyCSV = fullfile(proj.RootFolder, "healthy-data-all-temp.csv");
outfile = fullfile(proj.RootFolder, 'healthy_heart_sim_results_w_upd_vars.csv');
outfile_full = fullfile(proj.RootFolder, 'healthy_timeseries_extract_w_upd_vars.csv');


%T       = readtable(infile, 'PreserveVariableNames', true);
T = table(BSA, Age, HR, LVEDVi, LVESVi, RVEDVi, RVESVi, ...
    SBP, DBP, HI, ...
    IVCcomp, RVcomp, ...
    LVEF, RVEF, LVmassIdx, Eesep, ...
    O2PulsePct, VO2maxPct, CCI, SternalTor, RAsz, LAsz);
% Rename columns to match what runHeart_allPatients.m looks for
T.Properties.VariableNames = { ...
    'BSA','Age','HR', ...
    'LVEDVi (mL/m2)','LVESVi (mL/m2)','RVEDVi (mL/m2)','RVESVi (mL/m2)', ...
    'SBP','DBP','Mean HI', ...
    'IVC compression?','RV compression?', ...
    'LVEF%','RVEF%','LV mass index','E/e'' ratio (MV septal)', ...
    'O2 Pulse %predicted','VO2 max %predicted','CCI','Sternal Torsion Angle', ...
    'RA size','LA size'};

% Save synthetic file to project root
writetable(T, healthyCSV);
fprintf("Synthetic healthy table saved to:\n %s\n", healthyCSV);
